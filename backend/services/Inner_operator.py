...
def get_all():
    ...
    operators = []
    book_manage = Role.objects.filter(appellation='bookManager')
    reception = Role.objects.filter(appellation='reception')
    if not book_manage:
        return
    if not reception:
        return
    book_manage = Role.objects.get(appellation='bookManager')
...
def create(ID, role, password, name):
    ...
    inner_operator = User.objects.create_user(username=ID, password=password, role=role, name=name)
    inner_operator.save()
    return 0           #创建成功
